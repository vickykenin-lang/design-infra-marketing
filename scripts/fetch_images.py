#!/usr/bin/env python3
"""AURA — Image Researcher v5 (Pipeline V2, 2026-08-19).

Traffic-first redesign (Dr. Victor):
  Primary: Unsplash (when UNSPLASH_ACCESS_KEY is set)
  Fallback: Openverse → Wikimedia (strict modern-only)
  Last: Cloudflare AI concept fill → local SVG

Every non-real-project image is saved with concept=True so render_cards.mjs
shows an honest "Concept visualisation" badge. Real Design Infra project
photos (when supplied later) will carry real_project=True and skip the badge.
...
STRONG_HINT = (
    ...
    "modern interior", "apartment interior", "contemporary interior",
)
WEAK_HINT = (
...
Answer ONLY with compact JSON, no markdown fences: {{"is_real_interior_photo": true/false, "matches_room": true/false, "is_modern_photo": true/false, "is_aspirational": true/false, "has_watermark_or_logo": true/false, "reason": "<one short sentence>"}}
Rules:
...
def _save_credit(name, title, creator, license_, source, query, room, provider, ai_generated=False):
    credits[name] = {
        "title": title, "creator": creator or "",
        "license": license_ or "", "source": source or "",
        "query": query, "room": room, "provider": provider,
        "concept": True,          # all free/AI images are concept until real project photos arrive
    ...
    for r in results:
        ...
        try:
            ...
            if not ok and reason == "bad-keyword":
                ...
                continue
            # Unsplash regular ~1080px is good enough for IG cards
...
# Traffic-first queries — modern, aspirational, residential
JOBS = [
    ("modern living room interior design", "living", 8),
    ("contemporary living room apartment", "living", 6),
    ("modern modular kitchen interior", "kitchen", 8),
    ("modern kitchen cabinets design", "kitchen", 6),
    ("modern bedroom interior design", "bedroom", 8),
    ("cozy modern bedroom apartment", "bedroom", 6),
    ("modern home office interior desk", "office", 6),
    ("modern study room interior", "office", 4),
    ("modern bathroom interior design", "bathroom", 4),
]
